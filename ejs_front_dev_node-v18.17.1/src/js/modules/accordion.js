export function accordion() {
  ...
  //CODE：https://www.bring-flower.com/blog/accordion-menu/
  ...
  //アコーディオンコンテナ全てで実行
  const accordions = document.querySelectorAll('.js-acArea');
  accordions.forEach(function (accordion) {

    //アコーディオンボタン全てで実行
    const accordionBtns = accordion.querySelectorAll('.js-acBtn');
    accordionBtns.forEach( function(accordionBtn, index) {
      accordionBtn.addEventListener('click', function(e) {
        activeIndex = index; //クリックされたボタンを把握
        const isActive = e.target.parentNode.classList.toggle('is-active'); //ボタンの親要素(ul>li)にクラスを付与／削除
        ...
        //     accordionBtn.parentNode.classList.remove('is-active');
